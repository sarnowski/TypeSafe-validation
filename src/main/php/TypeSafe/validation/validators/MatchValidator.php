 ...
 */ 
class MatchValidator extends IsTextValidator {

    public function validate($value, $parameters) {
        parent::validate($value, $parameters);

        if (!isset($parameters['pattern'])) {
            throw new ValidationConfigurationException("No pattern given", $parameters);
        }

        $pattern = $parameters['pattern'];
        if (!preg_match($pattern, $value)) {
            throw new ValidationException($value, $parameters, "Value has to match the pattern $pattern");
        }
    }
}
