 ...
 */ 
class MaxNumberValidator extends IsNumericValidator {

    public function validate($value, $parameters) {
        parent::validate($value, $parameters);

        if (!isset($parameters['number'])) {
            throw new ValidationConfigurationException("Maximum number not configured", $parameters);
        }

        $number = $parameters['number'];
        if ($value > $number) {
            throw new ValidationException($value, $parameters, "Value must be maximum $number");
        }
    }
}
