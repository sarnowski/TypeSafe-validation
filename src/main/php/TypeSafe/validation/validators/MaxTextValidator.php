 ...
 */ 
class MaxTextValidator extends IsTextValidator {

    public function validate($value, $parameters) {
        parent::validate($value, $parameters);

        if (!isset($parameters['length'])) {
            throw new ValidationConfigurationException("Maximum length not configured", $parameters);
        }

        $length = $parameters['length'];
        if (strlen($value) > $length) {
            throw new ValidationException($value, $parameters, "Value must be longer than $length");
        }
    }
}
