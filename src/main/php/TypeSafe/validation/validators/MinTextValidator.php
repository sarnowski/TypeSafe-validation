...
require_once('IsTextValidator.php');
 ...
 */ 
class MinTextValidator extends IsTextValidator {

    public function validate($value, $parameters) {
        parent::validate($value, $parameters);

        if (!isset($parameters['length'])) {
            throw new ValidationConfigurationException("Minimum length not configured", $parameters);
        }

        $length = $parameters['length'];
        if (strlen($value) < $length) {
            throw new ValidationException($value, $parameters, "Value must be at least $length characters long");
        }
    }
}
